...
export const getSalesByPaymentMethodController = async (req, res) => {
    try {
        const { from, until } = req.query;
        if (
            !from ||
        ...
        const fromTimestamp = `${from}T00:00:00`;
        const untilTimestamp = `${until}T23:59:59`;
        const sales = await SaleModel.getSalesByPaymentMethod(
            fromTimestamp,
            untilTimestamp
        );
        if (sales.length === 0) {
...
export const getSalesByCategoryController = async (req, res) => {
    try {
        ...
        const fromTimestamp = `${from}T00:00:00`;
        const untilTimestamp = `${until}T23:59:59`;
        const sales = await SaleModel.getSalesByCategory(
            fromTimestamp,
            untilTimestamp
        );
        if (sales.length === 0) {
